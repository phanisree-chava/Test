...
import com.wellsafrgo.fsd.ims.dao.ItemDaojdbcImpl;
import com.wellsafrgo.fsd.ims.dao.Itemdao;
...
public class ItemServiceImpl implements ItemService {
	...
	private boolean isValidItem(Item item) throws ImsException {
		...
		if (!isFragileValid(item.getFargile())) {
			isValid = false;
			errMsg.add("Fragility can not be null");
		}

		if (!isValid) {
			throw new com.wellsfrago.fsd.ims.exception.ImsException(errMsg.toString());
		}
